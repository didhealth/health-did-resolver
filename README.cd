...
This DID method relies on the [health-did-registry](https://github.com/didhealthcare/health-did-registry).

## DID method

To encode a DID for an Ethereum address on the ethereum mainnet, simply prepend `did:health:`

eg:

`did:health:0xf3beac30c498d9e26865f34fcaa57dbb935b0d74`

Multi-network DIDs are also supported, if the proper configuration is provided during setup.

For example:
`did:health:0x4:0xf3beac30c498d9e26865f34fcaa57dbb935b0d74` gets resolved on the rinkeby testnet (chainID=0x4), and
...
```json
{
  ...
  "authentication": [
    "did:health:0xb9c5714089478a327f09197987f16f9e5d936e8a#controller"
  ],
  "assertionMethod": [
    "did:health:0xb9c5714089478a327f09197987f16f9e5d936e8a#controller"
  ]
...
the [health-did-registry](https://github.com/didhealthcare/health-did-registry) Ethereum smart contract.
...
import { getResolver } from 'health-did-resolver'

// While experimenting, you can set a rpc endpoint to be used by the web3 provider
// You can also set the address for your own health-did-registry contract
...
// getResolver will return an object with a key/value pair of { "health": resolver } where resolver is a function used by the generic did resolver.
const healthDidResolver = getResolver(providerConfig)
const didResolver = new Resolver(healthDidResolver)

didResolver.resolve('did:health:0xf3beac30c498d9e26865f34fcaa57dbb935b0d74').then((doc) => console.log)

// You can also use ES7 async/await syntax
const doc = await didResolver.resolve('did:health:0xf3beac30c498d9e26865f34fcaa57dbb935b0d74')
```
...
The configuration from above allows you to resolve health-did's of the following formats:

- `did:health:mainnet:0xabcabc03e98e0dc2b855be647c39abe984193675`
- `did:health:0xabcabc03e98e0dc2b855be647c39abe984193675` (defaults to mainnet configuration)
- `did:health:0x4:0xabcabc03e98e0dc2b855be647c39abe984193675` (refer to the rinkeby network by chainID)
- `did:health:rsk:testnet:0xabcabc03e98e0dc2b855be647c39abe984193675`
- `did:health:0x1f:0xabcabc03e98e0dc2b855be647c39abe984193675` (refer to the rsk:testnet by chainID)
- `did:health:development:0xabcabc03e98e0dc2b855be647c39abe984193675`
- `did:health:myprivatenet:0xabcabc03e98e0dc2b855be647c39abe984193675`
- `did:health:0x1e240:0xabcabc03e98e0dc2b855be647c39abe984193675` (refer to `myprivatenet` by chainID)
