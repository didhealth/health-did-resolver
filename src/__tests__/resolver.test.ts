...
import DidRegistryContract from 'health-did-registry'
...
describe('healthResolver', () => {
  // let registry, accounts, did, identity, controller, delegate1, delegate2, health, didResolver
  ...
  describe('unregistered', () => {
    ...
    it('resolves document with publicKey identifier', async () => {
      ...
      const pubdid = `did:health:dev:0x${pubKey}`
  ...
  describe('controller changed', () => {
    it('resolves document', async () => {
      ...
      await new HealthDidController(identity, registryContract).changeOwner(controller, { from: identity })
  ...
  describe('delegates', () => {
    describe('add signing delegate', () => {
      it('resolves document', async () => {
        expect.assertions(1)
        const blockHeightBeforeChange = (await web3Provider.getBlock('latest')).number
        await new HealthDidController(identity, registryContract).addDelegate('veriKey', delegate1, 86401, {
          from: controller,
        })
        const result = await didResolver.resolve(did)
    ...
    describe('add auth delegate', () => {
      it('resolves document', async () => {
        expect.assertions(1)
        const blockHeightBeforeChange = (await web3Provider.getBlock('latest')).number
        await new HealthDidController(identity, registryContract).addDelegate('sigAuth', delegate2, 1, {
          from: controller,
        })
        const result = await didResolver.resolve(did)
        expect(result).toEqual({
    ...
    describe('revokes delegate', () => {
      it('resolves document', async () => {
        expect.assertions(1)
        const blockHeightBeforeChange = (await web3Provider.getBlock('latest')).number
        await new HealthDidController(identity, registryContract).revokeDelegate('veriKey', delegate1, {
          from: controller,
        })
        await sleep(1000)
        const result = await didResolver.resolve(did)
    ...
    describe('re-add auth delegate', () => {
      it('resolves document', async () => {
        expect.assertions(1)
        const blockHeightBeforeChange = (await web3Provider.getBlock('latest')).number
        await new HealthDidController(identity, registryContract).addDelegate('sigAuth', delegate2, 86402, {
          from: controller,
        })
        const result = await didResolver.resolve(did)
        expect(result).toEqual({
  ...
  describe('attributes', () => {
    describe('add publicKey', () => {
      it('resolves with EcdsaSecp256k1VerificationKey2019', async () => {
        expect.assertions(1)
        await new HealthDidController(identity, registryContract).setAttribute(
      ...
      it('resolves with X25519KeyAgreementKey2019', async () => {
        expect.assertions(1)
        const keyAgrDid = `did:health:dev:${keyAgreementController}`
        await new HealthDidController(keyAgreementController, registryContract).setAttribute(
  ...
  describe('revoke publicKey', () => {
    it('resolves without EcdsaSecp256k1VerificationKey2019', async () => {
      expect.assertions(1)
      await new HealthDidController(identity, registryContract).revokeAttribute(
        'did/pub/Secp256k1/veriKey',
        '0x02b97c30de767f084ce3080168ee293053ba33b235d7116a3263d29f1450936b71',
        { from: controller }
      )
  ...
  describe('regression', () => {
    it('resolves same document with case sensitive eth address (https://github.com/decentralized-identity/health-did-resolver/issues/105)', async () => {
      expect.assertions(3)
      const lowAddress = accounts[5].toLowerCase()
      const checksumAddress = interpretIdentifier(lowAddress).address
      const lowDid = `did:health:dev:${lowAddress}`
      const checksumDid = `did:health:dev:${checksumAddress}`
      await new HealthDidController(lowAddress, registryContract).setAttribute(
    ...
    it('adds sigAuth to authentication section (https://github.com/decentralized-identity/health-did-resolver/issues/95)', async () => {
      expect.assertions(1)
      const address = accounts[4]
      const identifier = `did:health:dev:${address}`
      const authPubKey = `31303866356238393330623164633235386162353765386630646362363932353963363162316166`
      await new HealthDidController(identifier, registryContract).setAttribute(
    ...
    describe('Ed25519VerificationKey2018 in base58 (https://github.com/decentralized-identity/health-did-resolver/pull/106)', () => {
      it('resolves document', async () => {
        expect.assertions(1)
    ...
    describe('can deactivate a DID (https://github.com/decentralized-identity/health-did-resolver/issues/83)', () => {
      it('resolves deactivated document', async () => {
        expect.assertions(1)
        const address = accounts[6]
        const identifier = `did:health:dev:${address}`
        const blockHeightBeforeChange = (await web3Provider.getBlock('latest')).number
        await new HealthDidController(identifier, registryContract).changeOwner(nullAddress, { from: address })
        const result = await didResolver.resolve(identifier)
        expect(result).toEqual({
          didDocumentMetadata: {
            deactivated: true,
    ...
    describe('versioning', () => {
      it('can resolve virgin DID with versionId=latest', async () => {
        ...
        const virginDID = `did:health:dev:${virginAddress}`
      ...
      it('can resolve did with versionId before deactivation', async () => {
        expect.assertions(1)
        const deactivatedDid = `did:health:dev:${accounts[6]}`
      ...
      it('can resolve modified did with versionId=latest', async () => {
        ...
        await new HealthDidController(identifier, registryContract).changeOwner(address, { from: address })
      ...
      it('can resolve did with versionId before an attribute change', async () => {
        ...
        const blockHeightBeforeChange = (await web3Provider.getBlock('latest')).number
        const healthDid = new HealthDidController(identifier, registryContract)
        await healthDid.setAttribute('did/pub/Ed25519/veriKey/hex', `0x11111111`, 86411, { from: address })
        await healthDid.setAttribute('did/pub/Ed25519/veriKey/hex', `0x22222222`, 86412, { from: address })
      ...
      it('can resolve did with versionId before a delegate change', async () => {
        ...
        const healthDid = new HealthDidController(identifier, registryContract)
        const blockHeightBeforeChange = (await web3Provider.getBlock('latest')).number
        await healthDid.addDelegate('veriKey', delegateAddress1, 86401, { from: address })
        await healthDid.addDelegate('veriKey', delegateAddress2, 86402, { from: address })
      ...
      it('can resolve did with versionId before an owner change', async () => {
        ...
        const healthDid = new HealthDidController(identifier, registryContract)
        const blockHeightBeforeChange = (await web3Provider.getBlock('latest')).number
        await healthDid.changeOwner(address, { from: address })
        await healthDid.changeOwner(newOwner, { from: address })
        const result = await didResolver.resolve(`${identifier}?versionId=${blockHeightBeforeChange + 1}`)
        expect(result).toEqual({
          didDocumentMetadata: {
            versionId: `${blockHeightBeforeChange + 1}`,
      ...
      it('can resolve did with versionId before an attribute expiration', async () => {
        ...
        await new HealthDidController(identifier, registryContract).addDelegate('sigAuth', delegate, 1, {
          from: address,
        })
  ...
  describe('overlapping events', () => {
    it('adding the same service in the same block does not result in duplication', async () => {
      ...
      const tx1 = healthDid.setAttribute(stringToBytes32('did/svc/TestService'), 'https://test.uport.me', 86406, {
        from: address,
      })
      const tx2 = healthDid.setAttribute(stringToBytes32('did/svc/TestService'), 'https://test.uport.me', 86407, {
        from: address,
      })
      await sleep(1000)
    ...
    it('adding 2 services in 2 consecutive blocks should result in only 2 services appearing in the DID doc (no duplication)', async () => {
      ...
      // await stopMining(web3Provider)
      await healthDid.setAttribute(stringToBytes32('did/svc/TestService1'), 'https://test1.uport.me', 86406, {
        from: address,
      })
      // await startMining(web3Provider)
      let result = await didResolver.resolve(identifier)
      ...
      // await stopMining(web3Provider)
      await healthDid.setAttribute(stringToBytes32('did/svc/TestService2'), 'https://test2.uport.me', 86407, {
        from: address,
      })
      // await startMining(web3Provider)
    ...
    it('adding and removing a service in the same block should result in no change to the doc (correct order, same block)', async () => {
      ...
      await stopMining(web3Provider)
      const tx1 = healthDid.setAttribute(stringToBytes32('did/svc/TestService1'), 'https://test1.uport.me', 86406, {
        from: address,
      })
      let result = await didResolver.resolve(identifier)
      expect(result.didDocumentMetadata.versionId).not.toBeDefined()
      const tx2 = healthDid.revokeAttribute(stringToBytes32('did/svc/TestService1'), 'https://test1.uport.me', {
        from: address,
      })
    ...
    it('adding and removing a service in 2 consecutive blocks should result in no change to the doc (correct order 2 blocks).', async () => {
      ...
      let result = await didResolver.resolve(identifier)
      expect(result.didDocumentMetadata.versionId).toEqual(`${blockHeightBeforeChange + 1}`)
      await healthDid.revokeAttribute(stringToBytes32('did/svc/TestService1'), 'https://test1.uport.me', {
        from: address,
      })
