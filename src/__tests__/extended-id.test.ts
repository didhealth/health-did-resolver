...
describe('interpretIdentifier', () => {
  ...
  it('parses did:health with address', () => {
    const { address, publicKey, network } = interpretIdentifier(`did:health:${checksumAddress}`)
    expect(address).toEqual(checksumAddress)
    expect(publicKey).toBeUndefined()
    expect(network).toBeUndefined()
  })
  it('parses did:health with address and version', () => {
    const { address, publicKey, network } = interpretIdentifier(`did:health:${checksumAddress}?versionId=42`)
    expect(address).toEqual(checksumAddress)
    expect(publicKey).toBeUndefined()
    expect(network).toBeUndefined()
  })
  it('parses did:health with address and network', () => {
    const { address, publicKey, network } = interpretIdentifier(`did:health:0x1:${checksumAddress}`)
    expect(address).toEqual(checksumAddress)
    expect(publicKey).toBeUndefined()
    expect(network).toEqual('0x1')
  })
  it('parses did:health with address and sub-network', () => {
    const { address, publicKey, network } = interpretIdentifier(`did:health:rsk:testnet:${checksumAddress}`)
    expect(address).toEqual(checksumAddress)
    expect(publicKey).toBeUndefined()
    expect(network).toEqual('rsk:testnet')
  })
  it('parses did:health with address and sub-network and version', () => {
    const { address, publicKey, network } = interpretIdentifier(`did:health:rsk:testnet:${checksumAddress}?versionId=42`)
    expect(address).toEqual(checksumAddress)
    expect(publicKey).toBeUndefined()
    expect(network).toEqual('rsk:testnet')
  })
  ...
  it('parses did:health with publicKey', () => {
    const { address, publicKey, network } = interpretIdentifier(`did:health:${pubKey}`)
    expect(address).toEqual(checksumAddress)
    expect(publicKey).toEqual(pubKey)
    expect(network).toBeUndefined()
  })
  it('parses did:health with publicKey and version', () => {
    const { address, publicKey, network } = interpretIdentifier(`did:health:${pubKey}?versionId=42`)
    expect(address).toEqual(checksumAddress)
    expect(publicKey).toEqual(pubKey)
    expect(network).toBeUndefined()
  })
  it('parses did:health with publicKey and network', () => {
    const { address, publicKey, network } = interpretIdentifier(`did:health:mainnet:${pubKey}`)
    expect(address).toEqual(checksumAddress)
    expect(publicKey).toEqual(pubKey)
    expect(network).toEqual('mainnet')
  })
  it('parses did:health with publicKey and sub-network', () => {
    const { address, publicKey, network } = interpretIdentifier(`did:health:not:so:main:net:${pubKey}`)
    expect(address).toEqual(checksumAddress)
    expect(publicKey).toEqual(pubKey)
    expect(network).toEqual('not:so:main:net')
  })
  it('parses did:health with publicKey and sub-network', () => {
    const { address, publicKey, network } = interpretIdentifier(`did:health:not:so:main:net:${pubKey}?versionId=42`)
    expect(address).toEqual(checksumAddress)
    expect(publicKey).toEqual(pubKey)
    expect(network).toEqual('not:so:main:net')
  })
