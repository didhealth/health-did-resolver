...
describe('healthResolver (alt-chains)', () => {
  ...
  describe('eth-networks', () => {
    it('resolves on mainnet with versionId', async () => {
      ...
      const result = await resolver.resolve('did:health:0x26bf14321004e770e7a8b080b7a526d8eed8b388?versionId=12090174')
      expect(result).toEqual({
        ...
        didDocument: {
          ...
          id: 'did:health:0x26bf14321004e770e7a8b080b7a526d8eed8b388',
          verificationMethod: [
            {
              id: 'did:health:0x26bf14321004e770e7a8b080b7a526d8eed8b388#controller',
              type: 'EcdsaSecp256k1RecoveryMethod2020',
              controller: 'did:health:0x26bf14321004e770e7a8b080b7a526d8eed8b388',
              blockchainAccountId: 'eip155:1:0x26bF14321004e770E7A8b080b7a526d8eed8b388',
            },
          ],
          authentication: ['did:health:0x26bf14321004e770e7a8b080b7a526d8eed8b388#controller'],
          assertionMethod: ['did:health:0x26bf14321004e770e7a8b080b7a526d8eed8b388#controller'],
        },
      })
    })

    it('resolves on ropsten when configured', async () => {
      const did = 'did:health:ropsten:' + addr
      const health = getResolver({
        networks: [{ name: 'ropsten', rpcUrl: 'https://ropsten.infura.io/v3/6b734e0b04454df8a6ce234023c04f26' }],
      })
      const resolver = new Resolver(health)
    ...
    it('resolves on rinkeby when configured', async () => {
      const did = 'did:health:rinkeby:' + addr
      const health = getResolver({
    ...
    it('resolves on kovan when configured', async () => {
      const did = 'did:health:kovan:' + addr
      const health = getResolver({
    ...
    it('resolves on rsk when configured', async () => {
      const did = 'did:health:rsk:' + addr
      const health = getResolver({ networks: [{ name: 'rsk', rpcUrl: 'https://did.rsk.co:4444' }] })
      const resolver = new Resolver(health)
      const result = await resolver.resolve(did)
      expect(result).toEqual({
        didDocumentMetadata: {},
    ...
    it('resolves on rsk:testnet when configured', async () => {
      const did = 'did:health:rsk:testnet:' + addr
      const health = getResolver({ networks: [{ name: 'rsk:testnet', rpcUrl: 'https://did.testnet.rsk.co:4444' }] })
      const resolver = new Resolver(health)
      const result = await resolver.resolve(did)
      expect(result).toEqual({
        didDocumentMetadata: {},
    ...
    it('resolves public key identifier on rsk when configured', async () => {
      const did = 'did:health:rsk:0x03fdd57adec3d438ea237fe46b33ee1e016eda6b585c3e27ea66686c2ea5358479'
      const health = getResolver({ networks: [{ name: 'rsk', rpcUrl: 'https://did.rsk.co:4444' }] })
      const resolver = new Resolver(health)
    ...
    it('resolves public keys and services on aurora when configured', async () => {
      const did = 'did:health:aurora:0x036d148205e34a8591dcdcea34fb7fed760f5f1eca66d254830833f755ff359ef0'
      const health = getResolver({
        networks: [
          {
            name: 'aurora',
