...
export function getResolver(options: ConfigurationOptions): Record<string, DIDResolver> {
  return new HealthDidResolver(options).build()
}

export class HealthDidResolver {
  ...
  async getOwner(address: string, networkId: string, blockTag?: BlockTag): Promise<string> {
    ...
    return new HealthDidController(address, this.contracts[networkId]).getOwner(address, blockTag)
  }
    ...
    if (!fullId) {
      return {
        didResolutionMetadata: {
          ...
          message: `Not a valid did:health: ${parsed.id}`,
        },
        didDocumentMetadata: {},
        didDocument: null,
      }
  ...
  build(): Record<string, DIDResolver> {
    return { health: this.resolve.bind(this) }
  }
}
